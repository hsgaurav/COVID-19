function res = fitVirusCV19(getData,daynumber,varargin)
   ...
   % tp4 = 2*tm + date0; % enter final phase
    ...
    if plt
       ...
       % xlim([t(1),t(end)]+date0);
        ...
        if sf == 1000
            ylabel({'Total Infected Cases';'(x1000)'},'fontweight','bold')
        else
        ...
        %... red regin
        fa = 0.3;
        h = fill([tp1,tp3,tp3,tp1],[0 0 1100 1100],'r',...
            'FaceAlpha',fa,'EdgeColor','none');
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        
        %... yellow region
        h = fill([tp3,tp4,tp4,tp3],[0 0 1100 1100],'y',...
            'FaceAlpha',fa,'EdgeColor','none');
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        
        %... green region
        h = fill([tp4,www,www,tp4],[0 0 1100 1100],'g',...
            'FaceAlpha',fa,'EdgeColor','none');
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
        ...
        Ct(Ct<0) = 0;
        h = plot(t(1:end-1) + date0,Ct,'m','LineWidth',1);
        h.Annotation.LegendInformation.IconDisplayStyle = 'on';
        ...
        if nsp == 3
            ...
            %... plot prediction
            plot(tt1(1:end-1)+date0,dC100,'k','LineWidth',2);
            ...
            %... plot actual
            scatter(tt(1:end-1)+date0,(dC./C(1:end-1))*100,50,'b','filled')
...
function plotData(C,date0,country,plt)
    ...
    xlabel('Date')
    if sf == 1000
        ylabel('Total Infected (x1000 cases)','fontweight','bold')
    else
